...
///count of numbers x such that l<=x<=r ans distinct digit in x=max digit in x
vll digit;
ll sz,k,dp[10010][110][2];
ll yo(ll idx,ll sum,bool badha)
{
    if(idx==-1){
        return (sum==0);
    }
    ll &ret=dp[idx][sum][badha];
    if(ret!=-1&&badha!=1) return ret;
    ll ans=0;
    ll mxhere=badha?digit[idx]:9;
    for(ll i=0;i<=mxhere;i++){
        bool next_badha=(i==digit[idx]?badha:0);
        ans+=yo(idx-1,(sum==0&&i==0)?sum:(sum+i)%k,next_badha);
        ans%=mod;
    }
    if(badha==0) ret=ans;
    return ans;

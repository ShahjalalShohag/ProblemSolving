...
const ld  eps=1e-9;
set<ll> se;
ll cnt[mxn],a[mxn],flag[mxn];
int main()
{
    fast;
    ll i,j,k,n,m,b,c,x,y,z,t,q;
    cin>>n;
    for(i=0;i<n;i++) cin>>a[i],cnt[a[i]]++;
    for(i=1;i<=n;i++) if(cnt[i]==0) se.insert(i);
    cout<<se.size()<<nl;
    for(i=0;i<n;i++){
        if(cnt[a[i]]>1){
            if(*se.begin()<a[i]||flag[a[i]]) cout<<*se.begin()<<' ',cnt[a[i]]--,se.erase(*se.begin());
            else cout<<a[i]<<' ',flag[a[i]]=1;
        }
        else{
            if(flag[a[i]]==1) cout<<*se.begin()<<' ',se.erase(*se.begin());
            else cout<<a[i]<<' ';
        }
    }
    return 0;

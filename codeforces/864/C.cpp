...
{
    fast;
    ll i,j,k,n,m,a,b,c,x,y,z,t,q,gas,point,cur,ans=0,dir;
    cin>>n>>gas>>point>>k;
    if(point>gas) return cout<<-1,0;
    cur=gas-point;
    dir=1;
    k++;
    while(k--){
        if(dir==1){
            if(k==1){
                if(n-point>cur) cur=gas,ans++;
                if(n-point>cur) return cout<<-1,0;
                cout<<ans<<nl;
                return 0;
            }
            else{
                if(2*(n-point)>cur) cur=gas,ans++;
                if(2*(n-point)>cur) return cout<<-1,0;
                cur-=2*(n-point);
                dir^=1;
            }
        }
            ...
            if(k==1){
                if(point>cur) cur=gas,ans++;
                if(point>cur) return cout<<-1,0;
                cout<<ans<<nl;
                return 0;
            }
            else{
                if(2*(point)>cur) cur=gas,ans++;
                if(2*(point)>cur) return cout<<-1,0;
                cur-=2*point;
                dir^=1;
            }
        }

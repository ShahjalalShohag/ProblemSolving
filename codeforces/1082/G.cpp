...
struct dinic
{
	...
	{
		edge a={v,(ll)g[v].size(),0,w};
		edge b={u,(ll)g[u].size(),0,0};
...
int main()
{
    ...
    for(i=1;i<=m;i++){
        cin>>u>>v>>w;
        flow.addedge(i,u+m,1e18);
        flow.addedge(i,v+m,1e18);
        flow.addedge(s,i,w);
        sum+=w;
    }
    for(i=1;i<=n;i++) flow.addedge(i+m,t,a[i]);
    cout<<sum-flow.max_flow(s,t)<<nl;
    return 0;
}
/*
Before submit=>

...
{
    ld a1=x.F*1.0,a2=x.S*1.0,b1=y.F*1.0,b2=y.S*1.0,p,q;
    p=a2/a1;
    q=b2/b1;
    return p<=q;
}
ll yo(ll n,ll st)
{
    if(n==0) return 0;
    if(st==mx) return inf;
    if(n>=v[st].F) return (n/v[st].F)*v[st].S+yo(n%v[st].F,0);
    else return min(v[st].S,yo(n,st+1));
}
int main()
{
    fast;
    ll i,j,k,l;
    cin>>mx>>l;
    for(i=1;i<=mx;i++) cin>>k,v.pb(MP(1LL<<(i-1),k));
    sort(all(v),compare);
    cout<<yo(l,0)<<nl;
    return 0;
}

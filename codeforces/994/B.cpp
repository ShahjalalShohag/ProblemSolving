...
int main()
{
    ...
    cin>>n>>k;
    for(i=1;i<=n;i++) cin>>a[i].pow,a[i].i=i;
    for(i=1;i<=n;i++) cin>>a[i].coin;
    sort(a+1,a+n+1,[](ss p,ss q){
         if(p.pow==q.pow) return p.coin>q.coin;
         else return p.pow<q.pow;});
    for(i=1;i<=n;i++){
        ans+=a[i].coin;
        out[a[i].i]=ans;
        pq.push(a[i].coin);
        if(pq.size()>k) ans-=pq.top(),pq.pop();
    }

...
void linear_sieve()
{
    for(int i=2;i<mxn;i++){
        ...
        int sz=prime.size();
        for(int j=0;j<sz&&i*prime[j]<mxn&&prime[j]<=lps[i];j++) lps[i*prime[j]]=prime[j];
    }
}
void get_factors(int n,vpii &v)
{
    while(n>1){
        int x=lps[n],cnt=0;
        while(n%x==0) n/=x,cnt++;
...
void dfs(int u,int pre)
{
    ...
    vpii fact;
    get_factors(a[u],fact);
    for(auto x:fact){
        ll now=x.F;
        for(int i=1;i<=x.S;i++) ans[u][now]++,now*=x.F;
    }
    for(auto x:ans[par[u][0]]) ans[u][x.F]+=x.S;
...
int main()
{
    ...
    while(q--){
        ...
        for(auto x:fact){
            ll now=x.F,power=0;
            for(i=1;i<=x.S;i++){
                power+=ans[u][now]+ans[v][now]-ans[lc][now]-ans[par[lc][0]][now];
                now*=x.F;
            }
            res=res*qpow(x.F,power)%mod;
        }
        cout<<res<<nl;
    }
    return 0;

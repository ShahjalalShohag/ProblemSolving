...
int main()
{
    ...
    while(i<=n){
        ...
        while(i<=n&&a[i]<=a[i-1]) i++;
        for(j=prev;j<i;j++) m[j].F=k,m[j].S=k;
        j=i-2;
        m[i-1].S=k+1;
        while(j>=0&&a[j]==a[j+1]) m[j].S=k+1,j--;
        k++;
        prev=i;
    }
    while(q--) cin>>l>>r,m[l].F==m[r].F||m[l].F==m[r].S||m[l].S==m[r].F||m[l].S==m[r].S?cout<<"Yes\n":cout<<"No\n";
    return 0;
}

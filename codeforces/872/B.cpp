...
{
    fast;
    ll i,j,k,n,mx=-inf,mn=inf,ans=-inf;
    cin>>n>>k;
    for(i=0;i<n;i++) cin>>a[i],mx=max(mx,a[i]),mn=min(mn,a[i]);
    if(k>=3) return cout<<mx<<nl,0;
    if(k==1) return cout<<mn<<nl,0;
    m[n-1]=a[n-1];
    for(i=n-2;i>=0;i--) m[i]=min(m[i+1],a[i]);
    mn=inf;
    for(i=0;i<n-1;i++){
        mn=min(mn,a[i]);
        k=max(mn,m[i+1]);

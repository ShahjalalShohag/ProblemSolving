...
int maxval(int i,int val,int sum)
{
    node *cur=root[i];
    if(val%i!=0||cur->minval+val>sum) return -1;
    int ans=0;
    for(i=20;i>=0;i--){
        int k=(val>>i)&1;
        if(cur->next[k^1]!=NULL&&cur->next[k^1]->minval+val<=sum){
            cur=cur->next[k^1];
            ans+=(k^1)<<i;
        }
        else{
...
{
    fast;
    int i,j,k,n,m,t,u,s,q;
    for(i=1;i<mxn;i++) for(j=i;j<mxn;j+=i) d[j].pb(i);
    for(i=1;i<mxn;i++) root[i]=new node();
    cin>>q;
    while(q--){
        cin>>t;
        if(t==1){
            ...
            if(vis[u]) continue;
            vis[u]=1;
            for(auto x:d[u]) add(x,u);
        }
        else{
            cin>>n>>k>>s;
            cout<<maxval(k,n,s)<<nl;
        }
    }
    return 0;
}

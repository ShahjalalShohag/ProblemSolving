...
{
    BeatMeScanf;
    ll i,j,k=200,n,m,cc,f1,f2,f3;
    vll a({3,mod-2,0,mod-1,1});
    vll f({0,0,0,2,6});
    vll b({1,0,0});
    vll c({0,1,0});
    vll d({0,0,1});
    ...
    cin>>n>>f1>>f2>>f3>>cc;
    ll ans=1LL;
    ans=ans*qpow(cc,linearRec(f,a,n-1),mod+1)%(mod+1);
    ans=ans*qpow(f1,linearRec(b,tr,n-1),mod+1)%(mod+1);
    ans=ans*qpow(f2,linearRec(c,tr,n-1),mod+1)%(mod+1);
    ans=ans*qpow(f3,linearRec(d,tr,n-1),mod+1)%(mod+1);
    cout<<ans<<nl;
    return 0;
}
///Before submit=>

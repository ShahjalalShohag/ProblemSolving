...
ll v[5][110][110],vec[220][220],valid[220][220];
string s;
int main()
{
    fast;
    ...
    for(k=1;k<=4;k++){
        for(i=1;i<=n;i++){
            cin>>s;
            for(j=1;j<=n;j++) v[k][i][j]=s[j-1]-'0';
        }
    }
    for(i=1;i<=2*n;i++) for(j=1;j<=2*n;j++) valid[i][j]=(i+j)%2;
    vll p({1,2,3,4});
    ll ans=1e18;
    do
    {
        for(i=1;i<=n;i++) for(j=1;j<=n;j++) vec[i][j]=v[p[0]][i][j];
        for(i=1;i<=n;i++) for(j=1;j<=n;j++) vec[i][j+n]=v[p[1]][i][j];
        for(i=1;i<=n;i++) for(j=1;j<=n;j++) vec[i+n][j]=v[p[2]][i][j];
        for(i=1;i<=n;i++) for(j=1;j<=n;j++) vec[i+n][j+n]=v[p[3]][i][j];
        ll a=0,b=0;
        for(i=1;i<=2*n;i++) for(j=1;j<=2*n;j++) a+=(vec[i][j]!=valid[i][j]);
        for(i=1;i<=2*n;i++) for(j=1;j<=2*n;j++) b+=(vec[i][j]!=(!valid[i][j]));
        ans=min(ans,min(a,b));

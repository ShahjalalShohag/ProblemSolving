...
int main()
{
    ...
    ll i,j,k,m,p;
    cin>>n;
    for(i=1;i<=n;i++) cin>>a[i],v.pb({a[i],{-1,i}});
//    for(i=1;i<=n;i++){
//        for(j=i+1;j<=min(a[i],n);j++) if(a[j]>=i) ans++;
//    }
//    cout<<ans<<nl;
    for(i=1;i<=n;i++) v.pb({i,{i+1,min(n,a[i])}});
    sort(all(v),cmp);
    for(i=0;i<v.size();i++){
        if(v[i].S.F==-1) add(v[i].S.S);
        else p=max(0LL,query(v[i].S.S)-query(v[i].S.F-1)),ans=ans+p;
    }
    cout<<ans<<nl;
    return 0;
}

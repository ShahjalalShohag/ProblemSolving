...
{
    fast;
    ll i,j,k,n,m,q,cs,ce,x1,y1,x2,y2,tmp,l,r,mid;
    cin>>n>>m>>cs>>ce>>v;
    for(i=0;i<cs;i++) cin>>k,st.pb(k);
    for(i=0;i<ce;i++) cin>>k,el.pb(k);
    cin>>q;
    while(q--){
        cin>>x1>>y1>>x2>>y2;
        ll ans=1e18;
        ...
        if(cs!=0){
            ll ind=LB(all(st),y1)-st.begin()-1;
            if(ind>=0&&ind<cs) ans=min(ans,abs(x1-x2)+abs(y1-st[ind])+abs(y2-st[ind]));
            ind=LB(all(st),y1)-st.begin();
            if(ind>=0&&ind<cs) ans=min(ans,abs(x1-x2)+abs(y1-st[ind])+abs(y2-st[ind]));
        }
         if(ce!=0){
            ll ind=LB(all(el),y1)-el.begin()-1;
            if(ind>=0&&ind<ce) ans=min(ans,(abs(x1-x2)+v-1)/v+abs(y1-el[ind])+abs(y2-el[ind]));
            ind=LB(all(el),y1)-el.begin();
            if(ind>=0&&ind<ce) ans=min(ans,(abs(x1-x2)+v-1)/v+abs(y1-el[ind])+abs(y2-el[ind]));
        }
        cout<<ans<<nl;
    }

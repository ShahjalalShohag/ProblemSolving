...
ll a[mxn],ar[mxn],sum[mxn];
int main()
{
    fast;
    ll i,j,k,n,m,q,tmp=0,pre,l,r,mid;
    cin>>n>>q;
    for(i=1;i<=n;i++) cin>>a[i],sum[i]=sum[i-1]+a[i];
    for(i=1;i<=q;i++) cin>>ar[i];
    pre=1;
    for(i=1;i<=q;i++){
        l=pre,r=n;
        ll ans=pre-1;
        while(l<=r){
            mid=(l+r)/2;
            if(sum[mid]-sum[pre-1]-tmp<=ar[i]) ans=mid,l=mid+1;
            else r=mid-1;
        }
        if(ans==n){
            cout<<n<<nl;
            ...
            cout<<n-ans<<nl;
            tmp=ar[i]-(sum[ans]-sum[pre-1]-tmp);
            pre=ans+1;
        }
    }
    return 0;

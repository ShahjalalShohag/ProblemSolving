...
long double obj[300],cons[300];
int a[N];
int32_t main()
{
    int n=in(),k=in(),x=in();
    for(int i=1;i<=n;i++) a[i]=in();
    for(int i=0;i<n;i++) obj[i]=1.0*a[i+1];
    lp::init(n,obj,MAXIMIZE);
    for(int i=0;i<n;i++){
        if(i) cons[i-1]=0.0;
        cons[i]=1.0;
        lp::add_constraint(cons,1.0,LESSEQ);
    }
    for(int i=0;i<n;i++) cons[i]=1.0;
    lp::add_constraint(cons,1.0*x,EQUAL);
    for(int i=0;i<n;i++) cons[i]=0.0;
    for(int i=0;i<k-1;i++) cons[i]=1.0;
    for(int i=k-1;i<n;i++){
        cons[i]=1.0;

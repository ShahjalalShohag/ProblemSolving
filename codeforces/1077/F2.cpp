...
int main()
{
    ...
    for(i=0;i<n;i++) dp[i][0]=-1e9;
    dp[n][0]=0;
    int nw=0;
    for(i=1;i<=x;i++){
        for(j=1;j<k;j++) q.push(dp[j][nw]);
        for(j=0;j<=n;j++){
            if(j+k<=n) q.push(dp[j+k][nw]);
            ll mx=q.top();
            if(mx==-1e9) dp[j][nw^1]=-1e9;
            else dp[j][nw^1]=mx+1LL*a[j];
            q.pop();
        }
        nw^=1;
    }
    ll ans=-1e9;
    for(i=0;i<k;i++) ans=max(ans,dp[i][nw]);
    if(ans<0) ans=-1;
    cout<<ans<<nl;
    return 0;
}

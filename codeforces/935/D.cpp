...
const int mxn=1e5+9;
const ld  eps=1e-9;
ll n,m,mc,mi,a[mxn],b[mxn],dp[mxn];
ll qpow(ll n,ll k,ll mod)          {ll ans=1;n%=mod;while(k){if(k&1) ans=(ans*n)%mod;n=(n*n)%mod;k>>=1;}return ans%mod;}
ll yo(ll i)
{
    if(i==n) return 0;
    else if(dp[i]!=-1) return dp[i];
    else if(a[i]==0&&b[i]==0) return dp[i]=(mc+yo(i+1)*mi%mod)%mod;
    else if(a[i]==0&&b[i]) return dp[i]=((m-b[i])*mi%mod+yo(i+1)*mi%mod)%mod;
    else if(a[i]&&b[i]==0) return dp[i]=((a[i]-1)*mi%mod+yo(i+1)*mi%mod)%mod;
    else if(a[i]>b[i]) return dp[i]=1;
    else if(a[i]<b[i]) return dp[i]=0;
    else return dp[i]=yo(i+1);
}
int main()
{
    fast;

...
{
    fast;
    ld i,j,k,n,m,x1,y1,xc,yc,d,ansx,ansy,r,ansr,b,e,theta;
    cin>>r>>xc>>yc>>x1>>y1;
    d=sqrt(SQ(xc-x1)+SQ(yc-y1));
    if(d>=r) return cout<<fixed<<setprecision(10)<<xc<<' '<<yc<<' '<<r<<nl,0;
    ansr=(r+d)/2;
    if(xc==x1&&yc==y1) return cout<<fixed<<setprecision(10)<<xc+ansr<<' '<<yc<<' '<<ansr<<nl,0;
    theta=atan2(yc-y1,xc-x1);
    cout<<fixed<<setprecision(10)<<x1+ansr*cos(theta)<<' '<<y1+ansr*sin(theta)<<' '<<ansr<<nl;
    return 0;
}

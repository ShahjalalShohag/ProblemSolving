...
void kmp(vector<int> lps, vi s, vi p)
{
...
int main()
{
    ...
    int i,j,k,n,m;
    cin>>n>>k;
    vi aa(n),bb(k),a(n-1),b(k-1);
    for(i=0;i<n;i++) cin>>aa[i];
    for(i=0;i<k;i++) cin>>bb[i];
    if(k==1) return cout<<n<<nl,0;
    if(n<k) return cout<<0<<nl,0;
    for(i=0;i<n-1;i++) a[i]=aa[i+1]-aa[i];
    for(i=0;i<k-1;i++) b[i]=bb[i+1]-bb[i];
    vi lps=build_lps(b);
    kmp(lps,a,b);
    cout<<ans.size()<<nl;
    return 0;

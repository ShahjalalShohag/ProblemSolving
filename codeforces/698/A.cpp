...
int main()
{
    ...
    for(i=1;i<=n;i++){
        ...
        if(k==1) dp[i][1]=max(dp[i-1][0],dp[i-1][2])+1;
        else if(k==2) dp[i][2]=max(dp[i-1][0],dp[i-1][1])+1;
        else if(k==3) dp[i][1]=max(dp[i-1][0],dp[i-1][2])+1,dp[i][2]=max(dp[i-1][0],dp[i-1][1])+1;
    }
    cout<<n-max({dp[n][0],dp[n][1],dp[n][2]})<<nl;
    return 0;
}

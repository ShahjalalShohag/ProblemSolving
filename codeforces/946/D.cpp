...
int skip(int row,int k)
{
    int &ret=dp2[row][k];
    if(ret!=-1) return ret;
    int l,r,i;
    ret=inf;
    if(k==v[row].size()) return ret=0;
    for(i=0;i<=k;i++){
        l=v[row][i];
        r=v[row][v[row].size()-(k-i)-1];
        ret=min(ret,r-l+1);
    }
    return ret;
}
int yo(int row,int k)
{
    if(row>=n) return 0;
    int &ret=dp1[row][k];
    if(ret!=-1) return ret;
    ret=inf;
    int i,p=min(k,(int)v[row].size());
    for(i=0;i<=p;i++){
        ret=min(ret,skip(row,i)+yo(row+1,k-i));
    }
    return ret;
}
int main()
{
    ...
    for(i=0;i<n;i++){
        cin>>s;
        for(j=0;j<m;j++) if(s[j]=='1') v[i].pb(j);
    }
    mem(dp1,-1);
    mem(dp2,-1);
    cout<<yo(0,k)<<nl;

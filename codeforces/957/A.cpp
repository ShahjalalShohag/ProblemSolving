...
int main()
{
    ...
    string s;
    cin>>n>>s;
    for(i=0;i<n-1;i++) if(s[i]==s[i+1]&&s[i]!='?') return cout<<"No\n",0;
    for(i=0;i<n-1;i++) if(s[i]==s[i+1]&&s[i]=='?') return cout<<"Yes\n",0;
    for(i=0;i<n;i++){
        if(s[i]=='?'){
            if(i==0||i==n-1) return cout<<"Yes\n",0;
            if(s[i-1]==s[i+1]) return cout<<"Yes\n",0;
        }
    }
    cout<<"No\n";
    return 0;

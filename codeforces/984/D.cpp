...
ll a[mxn],sol[mxn][mxn],ans[mxn][mxn];
bool flag[mxn][mxn],flagg[mxn][mxn];
ll get(ll l,ll r)
{
    if(l==r) return sol[l][r];
    if(flag[l][r]) return ans[l][r];
    flag[l][r]=1;
    return ans[l][r]=max(sol[l][r],max(get(l,r-1),get(l+1,r)));
}
ll yo(ll i,ll j)
{
    if(i>j) return 0;
    if(i==j) return sol[i][j]=a[i];
    if(flagg[i][j]) return sol[i][j];
    flagg[i][j]=1;
    return sol[i][j]=yo(i+1,j)^yo(i,j-1);
}
int main()
{
    fast;
    ...
    for(i=1;i<=n;i++) for(j=1;j<=n;j++) sol[i][j]=yo(i,j);
    cin>>q;
    while(q--){
        cin>>l>>r;
        cout<<get(l,r)<<nl;

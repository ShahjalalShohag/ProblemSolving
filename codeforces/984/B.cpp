...
{
    ll ans=0;
    if(valid(i+1,j)&&(v[i+1][j]=='*') )ans++;
    if(valid(i+1,j+1)&&(v[i+1][j+1]=='*')) ans++;
    if(valid(i+1,j-1)&&(v[i+1][j-1]=='*')) ans++;
    if(valid(i-1,j)&&(v[i-1][j]=='*')) ans++;
    if(valid(i-1,j+1)&&(v[i-1][j+1]=='*')) ans++;
    if(valid(i-1,j-1)&&(v[i-1][j-1]=='*')) ans++;
    if(valid(i,j+1)&&(v[i][j+1]=='*')) ans++;
    if(valid(i,j-1)&&(v[i][j-1]=='*')) ans++;
    return ans;
}
int main()
{
    ...
    cin>>n>>m;
    string s;
    for(i=0;i<n;i++) cin>>s,v.pb(s);
    for(i=0;i<n;i++)
        for(j=0;j<m;j++){
        if(v[i][j]=='.'){
            if(cnt(i,j)>0)return cout<<"NO\n",0;
        }
        else if(v[i][j]>='1'&&v[i][j]<='8'){
            ll z=v[i][j]-'0';
            if(cnt(i,j)!=z) return cout<<"NO\n",0;
        }
    }
    cout<<"YES\n";
    return 0;

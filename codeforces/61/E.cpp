...
{
    ll pos=l,i=l,j=mid+1;
    while(i<=mid&&j<=r){
        if(a[i].x<a[j].x){
            cnt[a[i].ind][1]+=(j-mid-1);
            tmp[pos++]=a[i++];
        }
        else{
            cnt[a[j].ind][0]+=(mid-i+1);
            tmp[pos++]=a[j++];
        }
    }
    while(i<=mid) cnt[a[i].ind][1]+=(r-mid),tmp[pos++]=a[i++];
...
int main()
{
    ...
    ll i,j,k,n,m,ans=0;
    cin>>n;
    for(i=0;i<n;i++) cin>>a[i].x,a[i].ind=i;
    mergesort(0,n-1);
    for(i=0;i<n;i++) ans+=cnt[i][0]*cnt[i][1];
    cout<<ans<<nl;
    return 0;
}

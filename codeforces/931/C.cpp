...
{
    fast;
    ll i,j,k,n,m,mn=inf,mx=-inf,ans1,ans2,ans;
    cin>>n;
    for(i=0;i<n;i++) cin>>a[i],mp[a[i]]++,mn=min(mn,a[i]),mx=max(mx,a[i]);
    if(mx-mn<2){
        cout<<n<<nl;
        for(i=0;i<n;i++) cout<<a[i]<<' ';
        return 0;
    }
    ans1=2*min(mp[mn],mp[mx]);
    ans2=2*(mp[mn+1]/2);
    if(ans1>ans2){
        ans=ans1;
        mp[mn]-=ans1/2;
    ...
    cout<<n-ans<<nl;
    for(i=mn;i<=mx;i++) while(mp[i]--) cout<<i<<' ';
    return 0;
}

...
PQ<ll,vll,greater<ll>>q;
int main()
{
    fast;
    ll i,j,k,n,m,l=0,r=0,min_y,max_x,seg,ans=0;
    cin>>n>>k;
    for(i=0;i<n;i++) cin>>a[i].x>>a[i].y,a[i].pos=i+1;
    sort(a,a+n,cmp);
    for(i=0;i<k-1;i++) q.push(a[i].y);
    for(i=k-1;i<n;i++){
        q.push(a[i].y);
        max_x=a[i].x;
        min_y=q.top();
        q.pop();
        seg=min_y-max_x+1LL;
        if(seg>ans) ans=seg,l=max_x,r=min_y;
    }
    if(ans==0){
        cout<<ans<<nl;
        ...
        cout<<ans<<nl;
        for(i=0;i<n&&k;i++){
            if(a[i].x<=l&&a[i].y>=r) cout<<a[i].pos<<' ',k--;
        }
    }
    return 0;
}

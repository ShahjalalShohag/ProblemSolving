...
pll now;
char s[110][110];
ll cnt[110],lef[110],righ[110],n,m,dp[110][110];
ll yo(ll i,pll now)
{
    ll jl,l,jr,r;
        if(i==n) return 0;
        if(cnt[i]==0) return yo(i+1,now);
        if(dp[now.F][now.S]!=-1) return dp[now.F][now.S];
        jl=righ[i];
        l=now.S+i-now.F+jl;
        jr=lef[i];
        r=m+1-now.S+i-now.F+m+1-jr;
        return dp[now.F][now.S]=min(l+yo(i+1,{i,jl}),r+yo(i+1,{i,jr}));
}
int main()
{
    fast;
    ll i,j,k,jl,jr,ans=0,l,r;
    mem(dp,-1);
    cin>>n>>m;
    for(i=n-1;i>=0;i--){
        cin>>s[i];
        for(j=0;j<m+2;j++) if(s[i][j]=='1') cnt[i]++;
        for(j=0;j<m+2;j++) if(s[i][j]=='1'){
            lef[i]=j;
            break;
        }
        for(j=m+1;j>=0;j--) if(s[i][j]=='1'){
            righ[i]=j;
            break;
        }
    }

...
char yo(ll n,ll k)
{
    ...
    k-=x.size();
    if(k<f[n-1]) return yo(n-1,k);
    k-=f[n-1];
    if(k<y.size()) return y[k];
    k-=y.size();
    if(k<f[n-1]) return yo(n-1,k);
    k-=f[n-1];
    if(k<z.size()) return z[k];
    return '.';
}
int main()
{
    fast;
    ll i,j,k,n,m,a,b,c,t,q;
    f[0]=s.size();
    for(i=1;i<56;++i) f[i]=x.size()+f[i-1]+y.size()+f[i-1]+z.size();
    for(i=56;i<mxn;i++) f[i]=LLONG_MAX;
    cin>>q;
    while(q--){
        cin>>n>>k;
        cout<<yo(n,k-1);

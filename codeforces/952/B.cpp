...
int main()
{
    ...
    ll i,j,k,m;
    string s;
    string n[]={"great","don't think so","don't touch me","not bad","cool"};
    string g[]={"no way","don't even","are you serious","go die in a hole","worse","terrible"};
    for(i=0;i<10;i++){
        cout<<i<<nl;
        getline(cin,s);
        if(s=="no"){
            mp[s]++;
            if(mp[s]>3) return cout<<"normal\n",0;
        }
        else{
            for(i=0;i<5;i++) if(n[i]==s) return cout<<"normal\n",0;
            return cout<<"grumpy\n",0;
        }
        flush(cout);
    }

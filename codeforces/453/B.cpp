...
int  a[101],dp[101][1<<17],mask[61],spf[61],ind[61],n,sol[101][1<<17];
vector<int> v;
void pre()
{
    int i,j,k;
    for(i=1;i<=60;i++) spf[i]=i;
    for(i=2;i<=60;i+=2) spf[i]=2;
    for(i=3;i*i<=60;i+=2) if(spf[i]==i) for(j=i+i;j<=60;j+=i) spf[j]=min(spf[j],i);
    for(i=2;i<=60;i++) if(spf[i]==i) v.pb(i);
    for(i=0;i<v.size();i++) ind[v[i]]=i;
    for(i=2;i<=60;i++){
        k=i;
        while(k>1){
            mask[i]|=(1<<ind[spf[k]]);
...
int yo(int pos,int x)
{
    ...
    for(int i=1;i<=60;i++){
        if(!(mask[i]&x)){
            int p=abs(a[pos]-i)+yo(pos+1,(mask[i]|x));
            if(p<ans) ans=p,sol[pos][x]=i;
        }
    }
    return ret=ans;
}

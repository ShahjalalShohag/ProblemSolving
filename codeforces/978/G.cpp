...
ll s[110],exam[110],prep[110],ans[110];
int main()
{
    fast;
    ll i,j,k,n,m;
    cin>>n>>m;
    for(i=1;i<=m;i++) cin>>s[i]>>exam[i]>>prep[i],ans[exam[i]]=m+1;
    for(i=1;i<=n;i++){
        if(ans[i]) continue;
        ll mn=110;
        for(j=1;j<=m;j++) if(prep[j]&&i>=s[j]&&i<exam[j]) if(exam[j]<mn) mn=exam[j],k=j;
        if(mn!=110) ans[i]=k,prep[k]--;
    }
    for(i=1;i<=n;i++) if(prep[i]) return cout<<-1<<nl,0;

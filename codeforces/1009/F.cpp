...
vi g[mxn];
int ans[mxn],col[mxn],sz[mxn],dep[mxn],mxval,cnt[mxn];
bool big[mxn];
set<int>se[mxn];
...
void add(int u,int pre,int x)
{
    if(x==-1)
    {
        se[cnt[dep[u]]].erase(dep[u]);
        if(mxval==cnt[dep[u]] && se[cnt[dep[u]]].empty())mxval--;
    }
    else
    {
        se[cnt[dep[u]]].erase(dep[u]);
        if(mxval==cnt[dep[u]])mxval++;
    }
    cnt[dep[u]]+=x;
    se[cnt[dep[u]]].insert(dep[u]);
    for(auto v:g[u]){
...
int main()
{
    ...
    dfs(1,0);
    int mxx=0;
    for(i=1;i<=n;i++)mxx=max(mxx,dep[i]);
    for(i=1;i<=mxx;i++)se[0].insert(i);
    dsu(1,0,1);
    for(i=1;i<=n;i++) cout<<ans[i]-dep[i]<<nl;
    return 0;
}

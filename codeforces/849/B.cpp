...
ll yo(ll x1,ll x2)
{
    ...
    ll i,j,k;
    bool ind[1100]={0};
    slope=(y[x1]-y[x2])/(x1*1.0-x2*1.0);
    ind[x1]=1;
    ind[x2]=1;
    for(i=1;i<=n;i++){
        if(i==x1||i==x2) continue;
        sl=(y[i]-y[x1])/(i*1.0-x1*1.0);
        if(sl==slope) ind[i]=1;
    }
    for(i=1;i<=n;i++){
        if(ind[i]==0){
    ...
    if(v.size()==0) return 0;
        ...
        slope2=(y[v[0]]-y[v[1]])/(v[0]*1.0-v[1]*1.0);
        for(i=2;i<v.size();i++){
            sl=(y[v[i]]-y[v[0]])/(v[i]*1.0-v[0]*1.0);
            if(fabs(sl-slope)>eps) return 0;
        }
    }
    if(fabs(slope-slope2)>eps) return 0;
    else return 1;
}
int main()
{

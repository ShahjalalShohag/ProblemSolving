...
{
    fast;
    ll i=0,j,k,n,m,a=0,b=0,c=0;
    string s;
    cin>>s;
    if(s.size()<3) return cout<<"NO\n",0;
    if(s[0]!='a') return cout<<"NO\n",0;
    while(i<s.size()&&s[i]=='a') i++,a++;
    if(i==s.size()||s[i]!='b') return cout<<"NO\n",0;
    while(i<s.size()&&s[i]=='b') i++,b++;
    if(i==s.size()||s[i]!='c') return cout<<"NO\n",0;
    while(i<s.size()&&s[i]=='c') i++,c++;
    if(i!=s.size()) return cout<<"NO\n",0;
    if(c==a||c==b) cout<<"YES\n";
    else cout<<"NO\n";
    return 0;
}

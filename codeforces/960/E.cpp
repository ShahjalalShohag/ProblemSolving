...
void dfs(ll u,ll pre)
{
    ...
    for(auto v:g[u]){
        ...
        dep[v]=dep[u]+1;
        dfs(v,u);
        if(childcnt) ans=(ans+((odd[u]*even[v]%mod-even[u]*odd[v]%mod+mod)%mod)*a[u]%mod+mod)%mod;
        childcnt++;
        odd[u]+=even[v];
        even[u]+=odd[v];
    }
...
int main()
{
    ...
    dfs(1,-1);
    for(i=1;i<=n;i++){
        if(dep[i]%2==0) ans=(ans+((odd[i]*(odd[1]-odd[i]+1)%mod-even[i]*(even[1]-even[i])%mod+mod)%mod)*a[i]%mod+mod)%mod;
        else ans=(ans+((odd[i]*(even[1]-odd[i]+1)%mod-even[i]*(odd[1]-even[i])%mod+mod)%mod)*a[i]%mod+mod)%mod;
    }
    ans=(ans*2)%mod;
    for(i=1;i<=n;i++) ans=(ans-a[i]+mod)%mod;
    cout<<ans<<nl;
    return 0;
}

...
int main()
{
    ...
    for(i=0;i<n;i++){
        fac[i]+=se.order_of_key(a[i]);//factoradic number
        se.erase(a[i]);
    }
    for(i=0;i<n;i++) cin>>b[i],se.insert(b[i]);
    for(i=0;i<n;i++){
        fac[i]+=se.order_of_key(b[i]);
        se.erase(b[i]);
    }
    for(i=n-1;i>=0;i--){
        if(i) fac[i-1]+=(fac[i]/(n-i));
        fac[i]%=(n-i);
    }
    for(i=0;i<n;i++) se.insert(i);
    for(i=0;i<n;i++){
        auto x=*se.find_by_order(fac[i]);
        cout<<x<<' ';
        se.erase(x);
    }
    return 0;

...
{
    fast;
    ll n,i,j,k,posmin=INT_MAX,negmax=INT_MIN,ans=0;
    cin>>n;
    rep(i,n){
        cin>>k;
        if(k>0){
            ans+=k;
            if(k&1) posmin=min(posmin,k);
        }
        else if(k<0&&k&1) negmax=max(negmax,k);
    }
    !(ans&1)?cout<<max(ans+negmax,ans-posmin):cout<<ans;
    cout<<nl;
    return 0;
}

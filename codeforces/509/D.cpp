...
{
    fast;
    ll n,i,j,k,a[110][110],m,ans=0;
    cin>>n>>m;
    for(i=1;i<=n;i++) for(j=1;j<=m;j++) cin>>a[i][j];
    for(i=1;i<=n;i++) for(j=2;j<=m;j++) ans=gcd(ans,a[i][j]-a[i][1]-a[1][j]+a[1][1]);
    if(ans==0) ans=inf;
    for(i=1;i<=n;i++) for(j=1;j<=m;j++) if(a[i][j]>=ans){
        cout<<"NO\n";
        return 0;
    }
    cout<<"YES\n"<<ans<<nl;
    ...
    cout<<nl;
    for(i=1;i<=m;i++) cout<<(a[1][i]-a[1][1]+ans)%ans<<' ';
    cout<<nl;
    return 0;
}

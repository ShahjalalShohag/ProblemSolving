...
const int mxn=1e5+9;
const ld eps=1e-9;
ll qpow(ll n,ll k) {ll ans=1;while(k>0){if(k&1) ans=(ans*n)%mod;n=(n*n)%mod;k>>=1;}return ans%mod;}

int main()
{
    fast;
    ll i,j,m,n,k,a,b,p,ans=1,sum=0,t,x;
    string s;
    cin>>n>>a>>b>>k>>s;
    for(i=0;i<k;i++){
        t=qpow(a,n-i)*qpow(b,i)%mod;
        if(s[i]=='+') sum=(sum+t)%mod;
        else sum=(sum-t+mod)%mod;
    }
    x=(n+1)/k-1;
    t=(b%mod)*qpow(a,mod-2)%mod;
    p=qpow(t,k)%mod;
    if(p==1){
        cout<<sum*(x+1)%mod<<nl;
        return 0;
    ...
    ans=qpow(p,x+1)%mod;
    ans=(ans-1+mod)%mod;
    ans=ans*qpow((p-1+mod)%mod,mod-2)%mod;
    cout<<(ans*sum%mod+mod)%mod<<nl;
    return 0;
}

...
int main()
{
    ...
    string s,ans="";
    cin>>n;
    for(i=0;i<200;i++) nex[i]=pre[i]=i;
    while(n--){
        se.clear();
        cin>>s;
        for(auto c:s) see.insert(c),se.insert(c);
        if(se.size()!=s.size()) return cout<<"NO",0;
        for(i=0;i<s.size()-1;i++){
            if((nex[s[i]]!=s[i]&&nex[s[i]]!=s[i+1])||(pre[s[i+1]]!=s[i+1]&&pre[s[i+1]]!=s[i])) return cout<<"NO",0;
            nex[s[i]]=s[i+1];
            pre[s[i+1]]=s[i];
        }
    }
    for(c='a';c<='z';c++){
        if(see.find(c)==see.end()) continue;
        if(pre[c]!=(int)c) continue;
        ch=c;
        while(nex[ch]!=(int)ch){
            ans+=ch;
            see.erase(ch);

...
{
    fast;
    ld i,j,k,n,m,h,c,d,ans,inc,dec,ini,cost;
    cin>>h>>m>>ini>>inc>>cost>>dec;
    if(h>=20) ans=cil(ini,dec)*cost*80/100;
    else ans=min(cil(ini,dec)*cost,cil(ini+(20*60-h*60-m)*inc,dec)*cost*80/100);

...
const int N = 2e5 + 9, M = N * 2 + N * 20 * 2;
...
int cnt[M];
long long sum[M], parsum[M];
int upd(int cur, int u) { //update node u in cur tree
	T++; assert(T < M);
	ct[T] = ct[cur]; cnt[T] = cnt[cur];
	sum[T] = sum[cur]; level[T] = level[cur];
	id[T] = id[cur]; parsum[T] = parsum[cur];
	cur = T;
	cnt[cur]++;
	sum[cur] += d[level[cur]][u];
	for(auto &v : ct[cur]) if(insub(v, u)) {
		v = upd(v, u);
		parsum[v] += d[level[cur]][u];
	}
	return cur;
}
long long query(int cur, int u) { //query on cur tree
	...
	while (id[cur] != id[u]) {
		int v = 0;
		for (auto x: ct[cur]) if(insub(x, u)) v = x;
		assert(v);
		ans += d[level[cur]][id[u]] * (cnt[cur] - cnt[v]);
		ans += sum[cur] - parsum[v];
		cur = v;
	}
	ans += sum[cur];
	return ans;

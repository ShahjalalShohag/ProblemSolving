...
{
    fast;
    ll i,j,k,n,m,que,l,x;
    cin>>n>>que;
    for(i=1;i<=n;i++) cin>>a[i];
    for(i=1;i<=que;i++) cin>>l>>x,q[l].pb({x,i});
    ll ans=1;
    se.insert(0);
    for(i=1;i<=n;i++){
        if(se.find(a[i])!=se.end()) ans=(ans*2)%mod;
        else for(auto x:se) se.insert(x^a[i]);
        for(auto x:q[i]) sol[x.S]=ans*(se.find(x.F)!=se.end());
    }
    for(i=1;i<=que;i++) cout<<sol[i]<<nl;
    return 0;
}

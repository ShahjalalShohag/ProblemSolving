...
const int mxn=2e5+9;
const ld eps=1e-9;
//int qpow(int n,int k) {int ans=1;while(k>0){if(k&1) ans=(ans*n)%mod;n=(n*n)%mod;k>>=1;}return ans;}
int a[mxn],tree[mxn][2],st[mxn],en[mxn],p;
bool parity[mxn];
vector<int> g[mxn];
...
int main()
{
    ...
    while(m--){
        ...
        if(t==1){
            cin>>x>>val;
            upd(st[x],parity[x],val);
            upd(en[x]+1,parity[x],-val);
        }
        else{
            cin>>x;
            u=query(st[x],0);
            v=query(st[x],1);
            if(parity[x]==1) cout<<a[x]+v-u<<nl;
            else cout<<a[x]+u-v<<nl;
        }
    }
    return 0;
}

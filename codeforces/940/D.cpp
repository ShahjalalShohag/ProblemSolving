...
{
    fast;
    ll i,j,k,n,m,l=-1000000000,r=1000000000;
    cin>>n;
    ...
    for(i=4;i<n;i++){
        if(b[i]=='1'){
            if(b[i-1]=='1') continue;
            if(b[i-1]=='0'&&b[i-2]=='0'&&b[i-3]=='0'&&b[i-4]=='0') l=max(l,max({a[i],a[i-1],a[i-2],a[i-3],a[i-4]})+1);
        }
        else{
            if(b[i-1]=='0') continue;
            if(b[i-1]=='1'&&b[i-2]=='1'&&b[i-3]=='1'&&b[i-4]=='1') r=min(r,min({a[i],a[i-1],a[i-2],a[i-3],a[i-4]})-1);
        }
    }
    if(r<l) r=1000000000;
    cout<<l<<' '<<r<<nl;

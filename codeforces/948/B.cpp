...
const int mxn=1e6+9;
const ld  eps=1e-9;
int mxpr[mxn],mnval[mxn];
int main()
{
    fast;
    int i,j,k,n,m,l,r,mid,ans=inf;
    for(i=2;i<mxn;i++) if(mxpr[i]==0) for(j=i;j<mxn;j+=i) mxpr[j]=i;
    for(i=2;i<mxn;i++) mnval[i]=(mxpr[i]==i?i:i-mxpr[i]+1);
    cin>>n;
    for(i=mnval[n];i<=n;i++){
        ans=min(ans,mnval[i]);
    }

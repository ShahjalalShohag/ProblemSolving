...
ll a[110][110],row[110],col[110],x,y;
int main()
{
    fast;
    ll i,j,k,n,m,t;
    cin>>n>>m;
    for(i=1;i<=n;i++) cin>>row[i],a[i][m]=row[i],x^=row[i];
    for(i=1;i<=m;i++) cin>>col[i],a[n][i]=col[i],y^=col[i];
    if(x!=y) return cout<<"NO\n",0;
    x^=row[n];
    y^=col[m];
    if((x^col[m])!=(y^row[n])) return cout<<"NO\n",0;
    a[n][m]=x^col[m];
    cout<<"YES\n";

...
int n;
ld point[2*mxn],seg[2*mxn];
ld yo(ld x)
{
    int id=LB(point+1,point+2*n+1,x)-point;
    ld ans=seg[id-1];
    if(id%2==0) ans+=x-point[id-1];
    return ans;
}
int main()
{
    ...
    sf("%d",&n);
    for(i=1;i<=n;i++){
        sf("%lf%lf",&point[2*i-1],&point[2*i]);
        seg[2*i-1]=seg[2*i-2];
        seg[2*i]=seg[2*i-1]+point[2*i]-point[2*i-1];
    }
    vect st(0,0);
    vect en((ld)1e9+10,0);
    sf("%d",&m);
    while(m--){
        sf("%lf%lf",&pt.x,&pt.y);
        vect l=line_intersection(st,en,pt,a);
        vect r=line_intersection(st,en,pt,b);
        ld ratio_=pt.y/(pt.y-s);
        pf("%.10f\n",(yo(r.x)-yo(l.x))/ratio_);
    }
    return 0;
}

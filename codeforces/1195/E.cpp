...
int32_t main()
{
    ...
    cin>>n>>m>>x>>y;
    cin>>g[0]>>p>>q>>mo;
    for(i=1;i<=n*m;i++) g[i]=(1LL*g[i-1]*p%mo+q)%mo;
    for(i=1;i<=n;i++) for(j=1;j<=m;j++) a[i][j]=g[(i-1)*m+(j-1)];
    for(i=1;i<=n;i++){
        monotonous_queue_max q;
        for(j=1;j<y;j++) q.push(-a[i][j]);
        for(j=y;j<=m;j++){
            q.push(-a[i][j]);
            mn[i][j]=-q.top();
            q.pop();

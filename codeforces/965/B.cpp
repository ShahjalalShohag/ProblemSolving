...
int main()
{
    ...
    for(i=1;i<=n;i++){
        for(j=p;j<=n;j++){
            fl=1;
            for(k=j-p+1;k<=j;k++) if(a[i][k]=='#') fl=0;
            if(fl) for(k=j-p+1;k<=j;k++) cnt[i][k]++;
        }
    }
    for(j=1;j<=n;j++){
        for(i=p;i<=n;i++){
            fl=1;
            for(k=i-p+1;k<=i;k++) if(a[k][j]=='#') fl=0;
            if(fl) for(k=i-p+1;k<=i;k++) cnt[k][j]++;
        }
    }
    for(i=1;i<=n;i++) for(j=1;j<=n;j++) if(cnt[i][j]>ans) ans=cnt[i][j],mx1=i,mx2=j;
    cout<<mx1<<' '<<mx2<<nl;
    return 0;
}

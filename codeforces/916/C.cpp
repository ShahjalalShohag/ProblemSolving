...
bool isprime(ll n)
{
    ...
    if(n%2==0) return 0;
    if(n==1) return 0;
    for(ll i=3;i<=sqrt(n);i+=2) if(n%i==0) return 0;
    return 1;
}
int main()
{
    fast;
    ll n,i,j,k,m,p,e,ans,mx=1000000000;
    cin>>n>>e;
    for(i=n;!isprime(i);i++){}
    ans=i;
    cout<<2<<' '<<ans<<nl;
    cout<<1<<' '<<n<<' '<<2<<nl;
    if(e==1) return 0;
    for(i=1;i<=n-3;i++) cout<<i<<' '<<i+1<<' '<<1<<nl;
    cout<<n-2<<' '<<n-1<<' '<<ans-2-(n-3)<<nl;
    e-=(n-1);
    j=1;
    while(e){
        if(j==n-1) cout<<j<<' '<<n<<' '<<mx,e--;
        for(i=j+2;i<=n&&e;i++){
            if(j==1&&i==n) break;
            cout<<j<<' '<<i<<' '<<mx<<nl;
            e--;

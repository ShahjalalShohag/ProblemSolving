...
int main()
{
    ll n,i,j,k,tmp,root,sq=0,nonsq=0,ans=0;
    vll sq_to_nonsq,nonsq_to_sq;
    cin>>n;
    rep(i,n){
        cin>>k;
        root=sqrt(k);
        if(root*root==k){
            ...
            tmp=min(k-root*root,(root+1)*(root+1)-k);
            nonsq_to_sq.pb(tmp);
        }
    }
    if(sq>n/2){
        srt(sq_to_nonsq);
        nonsq=n-sq;
        for(i=0;i<n/2-nonsq;i++) ans+=sq_to_nonsq[i];
    }
    else if(sq<n/2){
        srt(nonsq_to_sq);
        for(i=0;i<n/2-sq;i++) ans+=nonsq_to_sq[i];
    }
    else ans=0;
    cout<<ans<<nl;
    return 0;

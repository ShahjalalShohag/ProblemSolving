...
void sieve()
{
    int i,j,k,n,m;
    for(i=3;i*i<mxn;i+=2){
        if(ck[i]==0) for(j=i*i;j<mxn;j+=2*i) ck[j]=1;
    }
    v.pb(2);
    for(i=3;i<mxn;i+=2) if(ck[i]==0) v.pb(i);
}
bool valid(int n)
{
    for(int i=0;i<v.size()&&v[i]<=n;i++){
        if(n%v[i]==0&&v[i]<=p) return 1;
    }
    return 0;
}
int main()
{
    ...
    int i,j,k,n,m,y;
    cin>>p>>y;
    for(i=y;i>=max(p,y-100000);i--){
        if(valid(i)==0) return cout<<i<<nl,0;
    }
    cout<<-1<<nl;
    return 0;
}

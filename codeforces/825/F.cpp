...
{
    fast;
    ll i,j,k,n,m,yo,curhs,tmp,dp[mxn]={0},lps[mxn]={0},cnt[mxn]={0},len;
    string s;
    cin>>s;
    n=s.size();
    for(i=1;i<=n;i++) dp[i]=i+1,cnt[i]=(ll)log10l(i)+1;
    for(i=0;i<n;i++){
        kmp(s.substr(i),n-i,lps);
        for(j=1;j+i<=n;j++){
            len=j-lps[j];
            if(j%len==0) dp[i+j]=min(dp[i+j],dp[i]+cnt[j/len]+len);

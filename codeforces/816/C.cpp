...
ll a[110],b[110],g[110][110],ansa[110],ansb[110];
int main()
{
    fast;
    ll i,j,k,n,m,ans=1e18,flag=0;
    cin>>n>>m;
    for(i=1;i<=n;i++) for(j=1;j<=m;j++) cin>>g[i][j];
    for(a[1]=0;a[1]<=g[1][1];a[1]++){
        ll sum=0;
        for(i=1;i<=m;i++){
            b[i]=g[1][i]-a[1];
            if(b[i]<0) goto last;
        ...
        for(i=1;i<=n;i++) for(j=1;j<=m;j++) if(a[i]+b[j]!=g[i][j]) goto last;
        for(i=1;i<=n;i++) sum+=a[i];
        for(i=1;i<=m;i++) sum+=b[i];
        if(sum<ans){
            ans=sum;
            for(i=1;i<=n;i++) ansa[i]=a[i];
            for(i=1;i<=m;i++) ansb[i]=b[i];
            flag=1;
        }
        last:;
    }
    if(flag){
        cout<<ans<<nl;
        for(i=1;i<=n;i++) while(ansa[i]) ansa[i]--,cout<<"row "<<i<<nl;
        for(i=1;i<=m;i++) while(ansb[i]) ansb[i]--, cout<<"col "<<i<<nl;
    }
    else cout<<-1<<nl;
    return 0;
}

...
int main()
{
    ...
    while(m--){
        ...
        if(ch=='\n'){
            if(l<=r) cout<<query(1,0,n-1,l,r)<<nl;
            else cout<<min(query(1,0,n-1,0,r),query(1,0,n-1,l,n-1))<<nl;
        }
        else{
            sfl(x);
            if(l<=r) upd(1,0,n-1,l,r,x);
            else upd(1,0,n-1,0,r,x),upd(1,0,n-1,l,n-1,x);
        }
    }
    return 0;
}

...
bool cmp(sj x,sj y)
{
    if(abs(x.temp-y.temp)<eps) return x.val<y.val;
...
int main()
{
    ...
    int i,j,k,m;
    cin>>n>>T;
    for(i=0;i<n;i++) cin>>a[i].val,sum+=a[i].val;
    for(i=0;i<n;i++) cin>>a[i].temp;
    sort(a,a+n,cmp);
    ld l=0,r=sum,mid,ans=0.0;
    while(abs(r-l)>eps){
        mid=(l+r)/2.0;
        if(yo(mid)) ans=mid,l=mid;
        else r=mid;
    }

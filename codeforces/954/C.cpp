...
bool yo(ll m)
{
    ll prerow=1,precol=1;
    for(ll i=1;i<=n;i++){
        ll col=a[i]%m;
        if(col==0) col=m;
        ll row=(a[i]+m-1)/m;
        ll a=abs(row-prerow),b=abs(col-precol);
        if(i!=1) if(!(a==1&&b==0||a==0&&b==1)) return 0;
        prerow=row;
        precol=col;
    }
    return 1;
...
int main()
{
    ...
    for(i=2;i<=n;i++) se.insert(abs(a[i]-a[i-1]));
    if(se.size()>2) return cout<<"NO\n",0;
    if(*se.begin()==0) return cout<<"NO\n",0;
    if(se.size()==1) yo(*se.begin())?cout<<"YES\n"<<1000000000<<' '<<*se.begin()<<nl:cout<<"NO\n";
    else{
        if(*se.begin()!=1) return cout<<"NO\n",0;
        auto it=se.begin();
        ++it;
        if(yo(*it)) cout<<"YES\n"<<1000000000<<' '<<*it<<nl;
        else cout<<"NO\n";

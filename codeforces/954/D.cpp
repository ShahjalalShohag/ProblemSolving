...
void bfs(ll s,ll fl)
{
    ...
    if(fl==1) for(i=1;i<=n;i++) ds[i]=d[i];
    else for(i=1;i<=n;i++) dt[i]=d[i];
}
int main()
{
    fast;
    ll i,j,k,ans=0,u,v,m,s,t;
    cin>>n>>m>>s>>t;
    for(i=0;i<m;i++){
        cin>>u>>v;
        g[u].pb(v);
    ...
    bfs(s,1);
    bfs(t,2);
    for(i=1;i<=n;i++) for(j=i+1;j<=n;j++) if(e[i][j]==0&&ds[i]+dt[j]+1>=ds[t]&&ds[j]+dt[i]+1>=ds[t]) ans++;
    cout<<ans<<nl;
    return 0;
}

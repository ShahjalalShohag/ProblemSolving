...
int main()
{
    ...
    int i,j,k,n,m,q;
    cin>>n>>m;
    for(i=1;i<=m;i++) cin>>u[i]>>v[i]>>w[i],e[w[i]].eb(u[i],v[i]);
    cin>>q;
    for(i=1;i<=q;i++){
        cin>>k;
        while(k--){
    ...
    for(i=1;i<N;i++){
        ...
        for(auto x:Q[i]){
            if(x.F!=last){
                d.backtrack(sz(d.st)-presz);
                presz=sz(d.st);
                last=x.F;
            }
            if(d.connected(u[x.S],v[x.S])) ans[x.F]=-1;
            else d.unite(u[x.S],v[x.S]);
        }
        for(auto x:e[i])  d.unite(x.F,x.S);
    }
    for(i=1;i<=q;i++) (~ans[i]?cout<<"YES\n":cout<<"NO\n");
    return 0;
}
///Before submit=>

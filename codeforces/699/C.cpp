...
ll yo(ll i,ll prev)
{
    ...
    ll &ret=dp[i][prev];
    if(prev==0){
        if(a[i]==0) return ret=1+yo(i+1,0);
        else if(a[i]==1) return ret=yo(i+1,1);
        else if(a[i]==2) return ret=yo(i+1,2);
        else return ret=min(yo(i+1,1),yo(i+1,2));
    }
    else if(prev==1){
        if(a[i]==0) return ret=1+yo(i+1,0);
        else if(a[i]==1) return ret=1+yo(i+1,0);
        else if(a[i]==2) return ret=yo(i+1,2);
        else return ret=yo(i+1,2);
    }
    else{
        if(a[i]==0) return ret=1+yo(i+1,0);
        else if(a[i]==1) return ret=yo(i+1,1);
        else if(a[i]==2) return ret=1+yo(i+1,0);
        else return ret=yo(i+1,1);
    }
}
int main()
{

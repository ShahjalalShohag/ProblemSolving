...
int a[mxn],ans[mxn],id[mxn],n;
bool fl[mxn];
vector<int> v;
bool ck[20000];
void sieve()
{
    for(int i=3;i*i<20000;i+=2) if(ck[i]==0) for(int j=i*i;j<20000;j+=2*i) ck[j]=1;
    v.pb(2);
    for(int i=3;i<20000;i+=2) if(ck[i]==0) v.pb(i);
}
int reduce(int n)
{
    if(n==0) return 0;
    for(auto x:v){
        if(x*x>abs(n-0)) break;
        while(n%(x*x)==0) n/=(x*x);
    }
    return n;
}
...
int main()
{
    ...
    sieve();
    cin>>n;
    for(i=0;i<n;i++) cin>>a[i],a[i]=reduce(a[i]);
    compress();
    for(i=0;i<n;i++){
        mem(fl,0);
        int cnt=0;
        for(j=i;j<n;j++){
            if(fl[id[j]]==0&&a[j]!=0) cnt++,fl[id[j]]=1;
            ans[cnt]++;
        }
    }
    ans[1]+=ans[0];
